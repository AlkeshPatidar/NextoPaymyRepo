...
const TripDetailScreen = ({ navigation, route }) => {
    ...
    const trackingData = {
        ...
        distance: '1480km',
        steps: [
            { id: 1, title: 'Loading', time: '- 23:00', status: 'completed' },
            { id: 2, title: 'Intransit', time: '07-Dec 00:00', status: 'active' },
            { id: 3, title: 'Unloading', time: '', status: 'pending' },
            { id: 4, title: 'Delivered', time: '', status: 'pending' },
            { id: 5, title: 'PODVerified', time: '', status: 'pending' }
        ]
    };
    ...
    const trip = route?.params?.trip || {
        ...
        from: 'Bhiwandi',
        to: 'Panipat',
        trafficAgent: { name: 'Anjali Singh', languages: 'English, Hindi' },
        driver: { phone: '9650886512' },
        tracking: { location: 'Bhagan Toll Plaza', time: '07-Dec 23:12', status: 'Ontime' }
    };

    const documentsData = {
        loadingMemo: { title: 'Loading Memo', hasDownload: true, hasUpload: true },
        lr: { title: 'LR', fileCount: '1 Files', files: [{ id: 1, name: '6912255_L.jpg', type: 'image' }] },
        autoELR: { title: 'Auto E-LR', loading: true },
        manualELR: { title: 'Manual E-LR', hasAdd: true },
        pod: { title: 'POD', fileCount: '1 Files', files: [{ id: 1, name: '6912255_POD_20251.jpg', type: 'image' }] },
        invoice: {
            title: 'Invoice',
            billingAddress: 'AIRWAT LOGISTICS',
            location: 'Ahmedabad',
    ...
    const TripCard = () => (
        ...
        >
            ...
            <View style={styles.infoCardsRow}>
                <View style={[styles.infoCardSmall, { backgroundColor: isDarkMode ? '#333' : '#f0f0f0' }]}>
                    <MaterialCommunityIcons name="package-variant" size={16} color={App_Primary_color} />
                    <Text style={[styles.infoCardText, { color: isDarkMode ? '#ccc' : '#666' }]}>
                        {trip.weight}
                    </Text>
                </View>
                <View style={[styles.infoCardSmall, { backgroundColor: isDarkMode ? '#333' : '#f0f0f0' }]}>
                    <MaterialCommunityIcons name="map-marker-distance" size={16} color={App_Primary_color} />
                    <Text style={[styles.infoCardText, { color: isDarkMode ? '#ccc' : '#666' }]}>
                        {trip.distance}
                    </Text>
                </View>
                <View style={[styles.infoCardSmall, { backgroundColor: isDarkMode ? '#333' : '#f0f0f0' }]}>
                    <MaterialCommunityIcons name="currency-inr" size={16} color={App_Primary_color} />
                    <Text style={[styles.infoCardText, { color: isDarkMode ? '#ccc' : '#666' }]}>
                        {trip.amount}
                    </Text>
                </View>
            ...
            {/* Material Type */}
            <Text style={[styles.materialText, { color: isDarkMode ? '#999' : '#666' }]}>
                {trip.material}
            </Text>
            ...
            <Row style={{gap:10}}>
                ...
                <Text style={[styles.routeLocation, { color: isDarkMode ? white : '#000' }]}>
                    {trip.from}
                </Text>
            ...
            >
                ...
                <MaterialCommunityIcons name="chevron-right" size={16} color={App_Primary_color} />
            </TouchableOpacity>
        </LinearGradient>
    );

    const ContactCard = ({ title, subtitle, phone, isDriver }) => (
        <View style={[styles.contactCard, { backgroundColor: isDarkMode ? dark33 : white }]}>
            <View style={styles.contactIconWrapper}>
                <LinearGradient
                    colors={[App_Primary_color + '20', App_Primary_color + '10']}
                    style={styles.contactIconGradient}
                >
                    <MaterialCommunityIcons
                        name={isDriver ? "steering" : "account-tie"}
                        size={24}
            ...
            <View style={styles.contactInfoSection}>
                <Text style={[styles.contactLabel, { color: isDarkMode ? '#999' : '#666' }]}>
                    {title}
                </Text>
                <Text style={[styles.contactName, { color: isDarkMode ? white : '#000' }]}>
                    {subtitle}
                ...
                {phone && (
                    <Text style={[styles.contactPhone, { color: isDarkMode ? '#999' : '#666' }]}>
                        {phone}
                    </Text>
                )}
            </View>
    ...
    const ActionCard = ({ icon, title, subtitle, onPress }) => (
        <TouchableOpacity
            style={[styles.actionCard, { backgroundColor: isDarkMode ? dark33 : white }]}
            onPress={onPress}
            activeOpacity={0.7}
        >
            <View style={styles.actionIconWrapper}>
                ...
                >
                    <MaterialCommunityIcons name={icon} size={24} color={App_Primary_color} />
                </LinearGradient>
            </View>
            <View style={styles.actionContent}>
                <Text style={[styles.actionTitle, { color: isDarkMode ? white : '#000' }]}>
                    {title}
                </Text>
                {subtitle && (
                    <Text style={[styles.actionSubtitle, { color: isDarkMode ? '#999' : '#666' }]} numberOfLines={1}>
                        {subtitle}
                    </Text>
                )}
            </View>
            <MaterialCommunityIcons name="chevron-right" size={24} color={isDarkMode ? '#666' : '#999'} />
        </TouchableOpacity>
    );
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                <View style={styles.contentContainer}>
                    ...
                    <ActionCard
                        ...
                        title="Live Tracking"
                        subtitle={`${trip.tracking.location} • ${trip.tracking.status}`}
                        onPress={() => navigation.navigate('TrackingScreen')}
                    />
